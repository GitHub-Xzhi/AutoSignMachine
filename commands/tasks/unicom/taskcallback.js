...
var taskcallback = {
  ...
  query: async (axios, options) => {
    ...
    if (data.code === "0000") {
      console.log(
        data.timeflag === "1"
          ? `今日参加活动已达上限(${data.achieve}/${data.allocation}次)`
          : `活动可参加(${data.achieve}/${data.allocation}次)`
      );
      return {
  ...
  reward: async (axios, options) => {
    ...
    var data = {
      ...
      sdk_version: "3.3.0.3",
      user_agent:
        "Mozilla/5.0 (Linux; Android 9; VKY-AL00 Build/HUAWEIVKY-AL00; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.198 Mobile Safari/537.36",
      extra: {
        // "ad_slot_type": 7,
